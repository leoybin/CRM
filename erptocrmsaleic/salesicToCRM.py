...
class ERP2CRM():
    ...
    def get_saleorder(self):
        sql = """
            select FBillNo from RDS_CRM_SRC_saleOrderList where FIsDo = 3
        """
...
-- 	rec.F_SZSP_FaPLX,
	salesic.FINVOICENO,--票据号码
	rec.F_SZSP_XSLX, --票据类别
	rec.FBILLNO as invoice_no  -- 发票编码
...
FROM
	...
	LEFT JOIN T_IV_SALESICENTRY salesicentry ON salesicentry.FSRCBILLNO = rec.FBILLNO
	left join T_IV_SALESIC salesic on salesic.FID = salesicentry.FID
                    where saleorder.FBILLNO = '{}'  and saleout.FBILLNO !='' and rec.FDOCUMENTSTATUS = 'C'
                """.format(d['FBillNo'])
            ...
            if not data.empty:
                ...
                print(data)
                sql = "update a set a.FisDo=1 from RDS_CRM_SRC_saleOrderList a where FBillNo = '{}'".format(
                    d['FBillNo'])
